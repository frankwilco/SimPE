...
IPEndPoint(Dns.Resolve(remoteHost).AddressList[0], remotePort);
     ...
     {   
            string flname = Path.Combine(Path.GetTempPath(), Path.GetFileName(remoteFileName));
            if (System.IO.File.Exists(flname)) File.Delete(flname);
            File.Copy(fileName, flname);
     ...
     public delegate void UploadProgressEventHandler(string filename, long size, long sent, ref bool abort);
     ...
     public void upload(string fileName,bool resume)
     {
       ...
       while ((bytes = input.Read(buffer,0,buffer.Length)) > 0)
       {
         ...
         transfered += bytes;
         if (UploadProgress != null) UploadProgress(Path.GetFileName(fileName), input.Length, Math.Max(0, Math.Min(input.Length, transfered)), ref abort);
         if (abort) break;
       }
       input.Close();
