...
{
	/// <remarks>
	/// Rewrite of standard SimBrowser that exposes the 
	/// selected sim's SDesc instance.
	/// </remarks>
	public class SimBrowser : System.Windows.Forms.Form
	{
		...
		/// <summary>
		/// An <see cref="SDesc"/> instance of the selected sim (if any).
		/// </summary>
		public SDesc SelectedSim
		{
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(SimBrowser));
		...
		protected void AddSim(ExtSDesc sdesc) 
		{
			...
			lvi.SubItems.Add(new LocalizedLifeSections(sdesc.CharacterDescription.LifeSection).ToString());
			...
			lvi.SubItems.Add(String.Format("0x{0:X8}", sdesc.FileDescriptor.Instance));
			...
			lvi.SubItems.Add(String.Format("0x{0:X8}", sdesc.SimId));
		...
		protected void FillList()
		{
			...
			foreach(Interfaces.Files.IPackedFileDescriptor spfd in pfds) 
			{
				...
				if (!this.cbNpc.Checked && sdesc.IsNPC)
					continue;
				
				if (!this.cbTownie.Checked && sdesc.IsTownie)
					continue;
		...
		public IToolResult Execute(ref SimPe.Interfaces.Files.IPackedFileDescriptor pfd, ref SimPe.Interfaces.Files.IPackageFile package, Interfaces.IProviderRegistry prov) 
		{
			...
			/* 
			 * Commented out to save us from a conflicting dll reference (simpe.toolbox or something)
			 * 
		...
		private void Open(object sender, System.EventArgs e)
		{
			...
			// set the selected sim before leaving.
			this.selectedSim = (SDesc)lv.SelectedItems[0].Tag;
			this.pfd = this.selectedSim.FileDescriptor;
