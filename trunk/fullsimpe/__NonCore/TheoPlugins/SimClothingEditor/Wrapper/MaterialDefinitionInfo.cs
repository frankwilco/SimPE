...
namespace SimPe.Plugin.Wrapper
{
	...
	{
		GenericRcol baseTexture;
		bool baseTextureAvailable = true;
		...
		public GenericRcol BaseTexture
		{
			get
			{
				//return null;
				if (this.baseTexture == null && this.baseTextureAvailable)
				{
					string txtrName = this.BaseTextureName;
					if (!txtrName.EndsWith("_txtr", StringComparison.InvariantCultureIgnoreCase))
						txtrName += "_txtr";

					IScenegraphFileIndexItem item = FileTableBase.FileIndex.FindFileByName(txtrName, Utility.DataType.TXTR, MetaData.LOCAL_GROUP, true);
					if (item != null)
					{
						this.baseTexture = new GenericRcol(null, false);
						this.baseTexture.ProcessData(item, false);
					}
					else
						// don't try again
						this.baseTextureAvailable = false;
				}
