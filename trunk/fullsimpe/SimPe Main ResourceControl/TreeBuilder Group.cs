...
namespace SimPe
{
	public class GroupTreeBuilder  :InstanceGroupTreeBuilder
	{
		GroupResourceLister garl;

		internal GroupTreeBuilder(LoadedPackage pkg, ViewFilter filter, TreeView tv) : base(pkg, filter, tv)
		{
			garl = new GroupResourceLister(pkg, filter);
			garl.Finished += new EventHandler(OnFinishedThread);	
		}
